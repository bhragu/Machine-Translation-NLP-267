...
class CommonAttention(nn.Module):
  ...
  def forward(self, decoder_hidden, encoder_outputs):
    if self.method == "dot":
      ...
      fc_layer = self.fc(decoder_hidden+encoder_outputs)
      out = torch.tanh(fc_layer)
...
class AttnDecoderRNN(nn.Module):
  def __init__(self, hidden_size, output_size,batch_size, attention, n_layers=1, dropout_p=0.1):
    ...
    self.endi = nn.Linear(self.hidden_size*2, self.output_size)

  def forward(self, inputs, hidden, encoder_outputs):
    embed = self.embedding(inputs).view(1,1,-1)
    embed = self.dropout(embed)
    gru_output, hidden = self.gru(embed, hidden)
    align_sc = self.attention(gru_output,encoder_outputs)
    attention_w = F.softmax(align_sc.view(1,-1), dim=1)
    con_vec = torch.bmm(attention_w.unsqueeze(0),encoder_outputs)
    output = torch.cat((gru_output, con_vec),-1)
    ending_layer = self.endi(output[0])
    output = F.log_softmax(ending_layer, dim=1)
    return output, hidden, attention_w
...
def main():
    ...
    attn = CommonAttention(args.hidden_size, "general")
    decoder = AttnDecoderRNN(hidden_size=args.hidden_size, 
                             output_size=tgt_vocab.n_words, 
                             batch_size=args.batch_size, attention = attn,
